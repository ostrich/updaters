...
latest_link = sorted_links[-1]['href'].split('/gzdoom/')[-1]

# Print the version and build number of the file to be downloaded
version, build = latest_link.split('-')[2:4]
print(f"Downloading GZDoom version {version} build {build}...")

# Download the latest gzdoom archive file
response = requests.get(url + latest_link)

if response.status_code != 200:
    print(f"Error: Failed to download {url+latest_link}. Status code: {response.status_code}")
    exit()
